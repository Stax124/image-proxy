...
| `resize_algorithm` | `string` | Resize algorithm override: `lanczos3`, `bicubic`, `gaussian`, `thumbnail`, `auto`    |
| `bw`               | `0`/`1`  | Convert to black-and-white via luminance grayscale. Only `1` enables; other values leave color. |
...
# Black-and-white (luminance grayscale)
curl http://localhost:8000/photo.jpg?bw=1
...
- **Non-processable format** (SVG, ICO, GIF, BMP, TIFF, JXL) — The file is served as-is without processing, even if transformation parameters are present.
- **With transformations** — The image is decoded, processed through the pipeline and the result is cached if caching is enabled.
