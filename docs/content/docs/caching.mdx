...
- **Requests with transformations** (resize, format conversion, black-and-white) go through the decode → resize → optional B&W → encode pipeline. The result is cached to avoid repeating expensive operations.

The `X-Image-Proxy-Cache` response header indicates whether the response was served from cache:
- `X-Image-Proxy-Cache: HIT` — Served from cache
- `X-Image-Proxy-Cache: MISS` — Processed and (if eligible) stored in cache
...
Items larger than `CACHE_MAX_ITEM_SIZE` are not stored in the cache. The cache uses **Sieve eviction** and weights entries by `key_size + value_size` in bytes.
