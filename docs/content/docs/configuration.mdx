...
| `IMAGE_PROXY_USER_AGENT`   | `image-proxy/<version>` | `User-Agent` header sent when fetching [fallback images](#fallback) from an upstream. Falls back to the default when unset or empty. |
